document.addEventListener("DOMContentLoaded", () => {
    ...
    const translations = {
        en: {
            ...
            "alert.invalidDuration":
                "Please enter a valid timer duration (e.g., 5:00 or 30).",
        ...
        fr: {
            ...
            "alert.invalidDuration":
                "Veuillez entrer une durée valide (ex : 5:00 ou 30).",
            "confirm.resetSettings":
        ...
        es: {
            ...
            "alert.invalidDuration":
                "Por favor, ingrese una duración válida para el temporizador (ej. 5:00 o 30).",
            "confirm.resetSettings":
    ...
    function applyStaticTranslations() {
        ...
        document
            .querySelectorAll("[data-translate-title-key]")
            .forEach((el) => {
    ...
    function updateClock() {
        ...
        if (ampm && currentSettings.timeFormat === "12")
            titleTime += ` ${ampm}`;
        ...
        if (currentSettings.showDate) {
            ...
            try {
                ...
                console.warn(
                    `toLocaleDateString does not support language tag: ${currentSettings.language}. Using browser default.`
                );
            }
        }
    }
    ...
    function timerTick() {
        ...
        document.title = `(${formatTimeUnit(h)}:${formatTimeUnit(
            m
        )}:${formatTimeUnit(s)}) ${translate("timerTabTitlePrefix")}${translate(
            "tabTitleSuffix"
        )}`;
    ...
    function pauseTimer() {
        ...
        document.title = `${translate(
            "pausedTabTitlePrefix"
        )} ${originalDocTitle.replace(
            translate("tabTitleSuffix"),
            ""
        )}${translate("tabTitleSuffix")}`;
    }
    ...
    function finishTimer() {
        ...
        document.title = `${translate("timesUpTabTitle")}${translate(
            "tabTitleSuffix"
        )}`;
    ...
    function updateUIForAppMode() {
        ...
        if (appMode === "clock") {
            ...
            if (appMode === "timerSetup") {
                ...
                timerInputEl.focus();
                timerStartPauseBtn.setAttribute(
                    "data-translate-key",
                    "timer.start"
                );
    ...
    function applyTranslationsToDynamicElements() {
        if (timerStartPauseBtn.dataset.translateKey) {
            timerStartPauseBtn.textContent = translate(
                timerStartPauseBtn.dataset.translateKey
            );
        }
        if (timerResetBtn.dataset.translateKey) {
            timerResetBtn.textContent = translate(
                timerResetBtn.dataset.translateKey
            );
        }
        if (modeSwitchBtn.dataset.translateTitleKey) {
            modeSwitchBtn.title = translate(
                modeSwitchBtn.dataset.translateTitleKey
            );
        }
        timerInputEl.placeholder = translate("timer.inputPlaceholder");
    }
    ...
    function loadSettings() {
        ...
        if (savedSettings) {
            ...
            if (loaded.language && translations[loaded.language]) {
                currentSettings.language = loaded.language;
                userPreferredLanguageDetected = true;
            }
        }

        if (!userPreferredLanguageDetected) {
    ...
    // --- Action Button Fading Logic (REVISED) ---
    function updateActionButtonsVisibility(show) {
        if (show) {
            ...
            if (!settingsModal.classList.contains("show")) {
                // Don't hide if settings modal is open
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        const isMouseInCornerArea =
            e.clientX >= cornerRect.left - hoverPadding &&
            e.clientX <= cornerRect.right + hoverPadding &&
            e.clientY >= cornerRect.top - hoverPadding &&
            e.clientY <= cornerRect.bottom + hoverPadding + 50;

        if (isMouseInCornerArea) {
            clearTimeout(cornerHoverTimeout);
    ...
    actionButtonsContainer.addEventListener("mouseleave", () => {
        clearTimeout(cornerHoverTimeout);
        cornerHoverTimeout = setTimeout(() => {
            // Re-check based on the main mousemove logic which is more comprehensive
            // For simplicity here, we just initiate a hide if the mouse leaves the buttons themselves
            // The next mousemove event will re-evaluate if it's still in the broader corner.
            if (!settingsModal.classList.contains("show")) {
                updateActionButtonsVisibility(false);
            }
        }, 700);
    ...
    closeSettingsBtn.addEventListener("click", () => {
        ...
        cornerHoverTimeout = setTimeout(() => {
            // Start hide timer after modal closes
            updateActionButtonsVisibility(false);
        }, 700);
    });
    ...
    loadSettings();
    updateUIForAppMode();
    updateActionButtonsVisibility(false); // Start with buttons hidden
});
